      ...
      _toggleMegamenu: function ($item, e) {
        ...
        if (e.type === "mouseenter") {
          ...
          if ($megamenu.length && $megamenu.hasClass("show")) {
            ...
            $megamenu.slideUp({
              duration: Math.max(150, this.duration() * 0.4),
              start: function () {
      ...
      if ($languages.length && $weglot.length) {
        ...
        function insert() {
          var current_html = $weglot_current.find("a").html(),
            ...
            $li = $("<li>")
              .addClass("active")
              .attr("data-l", $weglot_current.attr("data-l"))
              .append($span);
        ...
        $header_list.on("click", "li:not(.active)", function () {
          ...
          $weglot_list
            .find('li[data-l="' + $this.attr("data-l") + '"] a')
            .trigger("click");
    ...
    widthLimitInit($container, namespace) {
      ...
      if ($elems.length) {
        this.handlerWidthLimit = theme.Global.responsiveHandler({
          ...
          events: {
            "load.widthlimit theme.resize.widthlimit header.widthlimit":
              function (e) {
                if (theme.current.is_desktop) {
                  $elems.each(function () {
                    var $elem = $(this),
                      ...
                      margin =
                        +$elem.attr("data-js-header-width-limit-margin") || 0,
                      $edge = $container
                        .find(
                          '[data-js-header-width-limit-edge="' + name + '"]'
                        )
                        .first(),
